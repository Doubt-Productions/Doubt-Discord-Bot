...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. The Guild `interactionCreate` router also registers on the same Discord event and skips work once another handler has replied or deferred.
...
- `src/handlers/events.js` special-cases `src/events/validations` as one `interactionCreate` listener and awaits each validator in file-list order.
- Callable exports in other direct event folders are grouped under the folder name. The `ready` folder uses this path, so `registerCommands.js` and `registerContextMenus.js` run sequentially on one `ready` listener.
- Object exports with `{ event, run }` register separate listeners for their declared Discord event. Files under `src/events/Guild` use this path for `messageCreate`, `guildMemberAdd`, `voiceStateUpdate`, and backup `interactionCreate` routing.
- `tests/events-handler-shape.test.js` guards the object-export path so a loader regression does not silently break prefix commands or Guild event modules.
...
3. Checkout uses `fetch-depth: 0`, so the workflow can inspect existing tags and generate a changelog from tag history.
4. It runs `git rev-parse v<version>` and skips release creation when that tag already exists.
...
- Any push to `main` with an untagged current version can create a release; release creation is not gated by a latest-commit version diff.
...
The `/rank` command stores XP data in the `xps` collection through `src/schemas/XpSchema.js`.

- `/rank info`, `/rank reset`, and `/rank set` all require the `user` option in `src/commands/slash/General/rank.js`.
- `/rank info` calls `interaction.options.getMember("user")`; if the target is not a guild member, it replies ephemerally and does not build a card.
- Rank card status is normalized by `src/utils/rankCardPresenceStatus.js` before calling `canvacord` `Rank#setStatus`. Missing presence, unknown statuses, and Discord's `invisible` status map to `offline`.
- `tests/rank-card-presence-status.test.js` guards the canvacord-safe status mapping. Keep it updated when rank-card presence behavior changes.
...
- Prefix command support depends on `config.handler.commands.prefix` and the `messageCreate` handler in `src/events/Guild/messageCreate.js`. The event loader registers that module by its declared `event` property.
