...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. Because validators run before the Guild fallback handlers, do not rely on `options.cooldown` unless the command is known to execute through `src/events/Guild/interactionCreate.js`.
...
- `src/events/validations/**` is special-cased into one sequential `interactionCreate` validator chain. Each validator file exports a callable function and receives `(client, ...args)`.
- Other function exports are grouped by folder name. Files under `src/events/ready/**` export functions, so they run sequentially from one `ready` listener.
- Object exports with `{ event, run }` register their own listener on `eventModule.event` and execute `eventModule.run(client, ...args)`. Files under `src/events/Guild/**` use this shape for `messageCreate`, `guildMemberAdd`, `voiceStateUpdate`, and fallback `interactionCreate` handlers.
- The object-export behavior is covered by `tests/events-handler-shape.test.js`; update that regression test when changing the loader contract.
...
- Prefix command support depends on the `messageCreate` handler in `src/events/Guild/messageCreate.js`, which is registered through the event loader's `{ event, run }` object-export path.
