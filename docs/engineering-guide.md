...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. The Guild `interactionCreate` handler is registered as a backup router, but it skips interactions already replied to or deferred by validators.
...
Event loading contracts:

- `src/handlers/events.js` treats `src/events/validations` as a special sequential `interactionCreate` validation chain.
- Function exports in other direct event folders are grouped under the folder name. For example, files under `src/events/ready` export functions and run when Discord emits `ready`.
- Object exports with `{ event, run }` are registered under `eventModule.event`, and the listener calls `eventModule.run(client, ...args)`. Guild handlers such as `messageCreate`, `guildMemberAdd`, `voiceStateUpdate`, and backup `interactionCreate` routing use this shape.
- The registration shape is covered by `tests/events-handler-shape.test.js`; update that test with the docs if adding a new event-module contract.
...
`.github/workflows/release.yml` publishes GitHub Releases from pushes to `main` when the derived `v<package.json version>` tag does not already exist.
...
3. It checks whether that tag already exists with `git rev-parse "$TAG"`.
...
- Bump `package.json` before merging to `main` when you want a release. The workflow does not create or commit version bumps, and it does not inspect the latest commit for a version-line diff.
- Non-version pushes to `main` are no-ops after the matching `v<version>` tag exists.
...
- Prefix command support depends on the `messageCreate` handler in `src/events/Guild/messageCreate.js`; it is registered through the Guild `{ event, run }` event-module contract and still requires `config.handler.commands.prefix` to be enabled.
