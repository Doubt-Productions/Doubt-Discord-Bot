...
- The validation path in `src/events/validations/chatInputCommandValidator.js` also calls chat-input commands directly, but it does not read `command.options.cooldown` or the `config.handler.commands.*` toggles used by the Guild handler. The Guild interaction handlers guard `interaction.replied || interaction.deferred`; the validators do not, so verify listener order and double-reply behavior before moving command execution between these paths.
...
- Files under `src/events/ready` and `src/events/validations` export callable functions and are run sequentially on their folder listener.
- Files under `src/events/Guild` export `{ event, run }` objects. Those modules are registered individually on `eventModule.event` and call `eventModule.run(client, ...args)`.
- `tests/events-handler-shape.test.js` covers the object-export branch and the special `validations` chain. Keep that test updated if you change the loader contract.
...
`.github/workflows/release.yml` publishes GitHub Releases from `main` when the tag derived from the current `package.json` version does not already exist.

Release workflow behavior:

1. A push to `main` starts the `Release` workflow.
2. The workflow checks out the full repository history and tags with `fetch-depth: 0`.
3. It reads `package.json` with Node and derives the release tag as `v<version>`, for example `v1.3.0`.
4. It runs `git rev-parse "$TAG"` and skips the rest of the job when that tag already exists.
5. If the tag is new, it sets up Node.js `22`, installs dependencies with `npm ci || npm install`, runs `npm test`, generates a changelog from commits since the latest version-sorted tag, and creates a non-draft, non-prerelease GitHub Release with `softprops/action-gh-release`.

Release operator notes:

- Bump `package.json` before merging when you want a new release. The workflow no longer checks whether the version line changed in the latest commit; it only checks whether `v<version>` already exists.
...
- Prefix command support depends on the `messageCreate` handler in `src/events/Guild/messageCreate.js` and `config.handler.commands.prefix` being enabled.
