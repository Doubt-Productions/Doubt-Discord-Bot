...
- The validation path in `src/events/validations/chatInputCommandValidator.js` also calls chat-input commands directly and does not apply the Guild handler cooldown map. Do not rely on `options.cooldown` unless the interaction is definitely routed through `src/events/Guild/interactionCreate.js`.
...
- The `validations` folder is a special case: it registers one `interactionCreate` listener and awaits each validator function sequentially.
- Object exports with `{ event, run }` register one listener for the module's declared Discord event and call `run(client, ...args)`. This is how `src/events/Guild/messageCreate.js`, `guildMemberAdd.js`, `jointocreate.js`, `interactionCreate.js`, and `components.js` are wired.
- Function exports are grouped by folder name and registered as one folder-scoped listener. This is how the `ready` folder registers ready-time command and context-menu synchronization.
- Keep export shapes consistent with the folder's contract. A `{ event, run }` module without an `event`, or a non-function validator, will not be routed correctly.
...
3. The checkout uses `fetch-depth: 0`, so tags are available to the release check and changelog generation.
4. It skips the release when the derived tag already exists.
...
- Bump `package.json` before the change lands on `main` when you want a release. The workflow does not create or commit version bumps, and it no longer inspects the latest diff to decide whether a release is needed.
...
- If `package.json` is bumped but the matching tag already exists, the workflow exits successfully without reinstalling dependencies, running tests, generating a changelog, or creating a release.
