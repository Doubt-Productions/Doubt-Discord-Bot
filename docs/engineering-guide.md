...
3. It checks whether the derived tag already exists with `git rev-parse`.
4. If the tag exists, the workflow sets `needed=false` and skips release setup, tests, changelog generation, and release creation.
...
- Bump `package.json` before merging when you want a new release tag. The workflow does not create or commit version bumps, and it no longer checks whether the latest commit changed the version line.
...
- Documentation-only or code-only pushes to `main` with an already-released `package.json` version should skip because the matching tag already exists.
