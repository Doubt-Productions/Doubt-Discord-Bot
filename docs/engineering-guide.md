...
- The validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. The Guild backup router skips interactions that are already replied or deferred, so `options.cooldown` is not enforced for the normal validator-handled slash-command path.
...
- The `validations` folder is a special `interactionCreate` listener that runs every validator file sequentially.
- Function exports are grouped under the folder name. This is how files under `src/events/ready` register on the Discord `ready` event.
- Object exports with `{ event, run }` register on `eventModule.event` and call `eventModule.run(client, ...args)`. This is how Guild modules such as `messageCreate.js`, `jointocreate.js`, and backup `interactionCreate` routers attach to Discord event names instead of the folder name.
- Keep `tests/events-handler-shape.test.js` passing when changing the loader; it guards the object-export behavior and the validation-chain special case.
...
`.github/workflows/release.yml` publishes GitHub Releases from pushes to `main` when the current package version does not already have a matching tag.
...
3. It checks whether `v<version>` already exists in the fetched tag set.
4. If the tag exists, the workflow skips the release.
...
- Bump `package.json` before merging when you want a new release. The workflow does not create or commit version bumps, and it does not inspect the latest commit for a version-line change.
...
- Any push to `main` can create a release if the current `package.json` version's tag is missing, even if that push did not edit `package.json`.
