...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. Depend on `options.cooldown` only for commands routed by the Guild backup handler; commands such as `/rob` that need stronger locking should keep their own in-command guard.
...
- Callable exports in other event folders, such as `src/events/ready/**`, are grouped under the folder name and run sequentially when that Discord event fires.
- Object exports with `{ event, run }`, such as files under `src/events/Guild/**`, register directly on `eventModule.event` and call `eventModule.run(client, ...args)`.
- Prefix commands, AFK checks, welcome messages, Join-to-Create voice handling, and the backup interaction/component routers all rely on that object-export path. Keep `tests/events-handler-shape.test.js` updated if the event loader contract changes.
...
3. Checkout uses `fetch-depth: 0`, so tags are available for the release gate and changelog.
...
- Any push to `main` can create a release if the current `package.json` version has no matching tag; the workflow does not inspect whether the latest commit changed the version line.
...
- Prefix command support depends on both `config.handler.commands.prefix === true` and the `messageCreate` handler in `src/events/Guild/messageCreate.js`.
