...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. Verify the active interaction path before depending on `options.cooldown` in production.
...
- Files under `src/events/ready` and `src/events/validations` export callable functions. Multiple callable exports in a folder run sequentially from one listener registered on the folder-derived event name, or on `interactionCreate` for `validations`.
- Files under `src/events/Guild` export `{ event, run }` objects. The loader registers these on `eventModule.event` and invokes `eventModule.run(client, ...args)`, so handlers such as `messageCreate` and the Guild `interactionCreate` router are not registered under the literal folder name `Guild`.
- `tests/events-handler-shape.test.js` guards the `{ event, run }` branch and the separate validation chain; update it when changing event module shapes.
...
`.github/workflows/release.yml` publishes GitHub Releases from `main` when the current package version does not already have a Git tag.

Release workflow behavior:

1. A push to `main` starts the `Release` workflow.
2. The workflow checks out full history, reads `package.json` with Node, and derives the release tag as `v<version>`, for example `v1.3.0`.
3. It checks whether the derived tag already exists with `git rev-parse "$TAG"`.
4. If the tag exists, the workflow sets `needed=false` and skips setup, install, tests, changelog generation, and release creation.
...
- Release creation is gated by `npm test`; keep tests passing before merging a version bump. The workflow does not inspect the latest commit for a version-line diff, so any push to `main` with a missing `v<package.json version>` tag can create the release.
...
- Prefix command support depends on the `messageCreate` handler in `src/events/Guild/messageCreate.js`, which is registered through the `{ event, run }` event-loader path. Verify that path if prefix commands stop responding.
