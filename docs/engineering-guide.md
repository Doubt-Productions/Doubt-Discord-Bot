...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. Verify the event loader behavior below before depending on `options.cooldown` in production.
...
- `src/handlers/events.js` registers `src/events/validations/**` as one sequential `interactionCreate` validation chain.
- Function exports in other event folders are grouped under the folder name. For example, files in `src/events/ready/**` export functions and run on the Discord `ready` event.
- Object exports with `{ event, run }` register on the module's declared `event` property. This is how `src/events/Guild/messageCreate.js`, `src/events/Guild/jointocreate.js`, and the Guild interaction backup handlers attach to `messageCreate`, `voiceStateUpdate`, and `interactionCreate`.
- The Guild `interactionCreate.js` and `components.js` handlers skip interactions that have already been replied to or deferred, so the validation chain remains the primary path for slash commands and components.
...
3. It checks whether that exact tag already exists in the fetched repository history.
4. If the tag exists, the workflow sets `needed=false` and skips the install, test, changelog, and release steps.
...
- Bump `package.json` before a push to `main` when you want a release. The workflow does not inspect the pushed diff for a version-line change; the tag existence check is the release gate.
...
- `tests/events-handler-shape.test.js`: the event loader must register `{ event, run }` Guild modules on their declared event names and keep validators on the shared `interactionCreate` chain.
- `tests/interaction-cooldown.test.js`: the Guild backup slash-command handler must keep command cooldowns scoped by user and command name.
- `tests/rank-card-presence-status.test.js`: rank cards must normalize Discord presence values before passing them to Canvacord.
...
- Prefix command support depends on `config.handler.commands.prefix` and the `messageCreate` handler in `src/events/Guild/messageCreate.js`; commands are loaded even when the runtime router is disabled.
