...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map, so `options.cooldown` only affects the backup Guild interaction router.
...
- `src/handlers/events.js` treats `src/events/validations` as a special sequential `interactionCreate` validation chain. Every validator function returned by `src/utils/getAllFiles.js` is awaited for each interaction.
- Other function exports are grouped by their direct folder name, so files under `src/events/ready` run sequentially on the Discord `ready` event.
- Object exports with `{ event, run }`, such as files in `src/events/Guild`, register on the module's declared `event` property and call `eventModule.run(client, ...args)`. Use this shape for Discord event names that do not match the folder name, such as `messageCreate`, `guildMemberAdd`, and `voiceStateUpdate`.
- `tests/events-handler-shape.test.js` guards the object-export path and the validations special case; update that test when changing loader behavior.
...
3. It checks whether the derived tag already exists in the full checkout with `git rev-parse "$TAG"`.
4. If the tag exists, the workflow sets `needed=false` and skips install, tests, changelog generation, and release creation.
...
- A release can also run when `package.json` was not changed if the current `v<package.version>` tag is missing. The tag-existence check, not a version-line diff, is the release gate.
...
- Prefix command support depends on the `messageCreate` handler in `src/events/Guild/messageCreate.js` and on `config.handler.commands.prefix`; the event loader registers the handler through its `{ event: "messageCreate", run }` export.
