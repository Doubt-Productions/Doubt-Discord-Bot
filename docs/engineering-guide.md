...
- The active validation path in `src/events/validations/chatInputCommandValidator.js` calls chat-input commands directly and does not apply the Guild handler cooldown map. If a slash command is handled by the primary validator, `options.cooldown` is skipped because the backup Guild handler returns after `interaction.replied || interaction.deferred`.
...
- The `validations` folder is special-cased to one sequential `interactionCreate` listener. Each validator file exports a callable function and is awaited in filesystem order.
- Function exports in other folders are grouped under the folder name as the Discord event name. `src/events/ready/**` uses this path, so ready handlers are awaited sequentially on one `ready` listener.
- Object exports with `{ event, run }` are registered individually on `eventModule.event` and call `eventModule.run(client, ...args)`. `src/events/Guild/**` uses this shape for `messageCreate`, `guildMemberAdd`, `voiceStateUpdate`, and backup `interactionCreate` routing.
- `tests/events-handler-shape.test.js` guards the object-export and validation special-case behavior. Run `npm test` after changing the loader or moving event files between folders.
...
3. `actions/checkout` fetches full history and tags with `fetch-depth: 0`.
4. The workflow runs `git rev-parse v<version>` and skips the release when the derived tag already exists.
...
- Bump `package.json` before merging when you want a new release. The workflow does not inspect the latest commit diff for a version-line change; any push to `main` can publish the current package version if the matching tag is absent.
...
- Prefix command support depends on `config.handler.commands.prefix` and the `messageCreate` handler in `src/events/Guild/messageCreate.js`; the event loader registers that Guild object handler by its declared `event`.
