...
2. **Context menus** — `src/events/ready/registerContextMenus.js` creates missing context menus and deletes menus marked `deleted` on `DEV_GUILD_ID`; it does not edit existing menus
...
| `rankCardPresenceStatus.js` | Normalize Discord presence values before `canvacord` rank-card rendering |
