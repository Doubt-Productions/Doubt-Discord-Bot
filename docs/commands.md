...
| `/rank info <user>` | View XP rank card | `user` (required): must be a member of the current server |
...
- **Ticket System** — category, stored channel setting, support role

Join-to-Create is displayed in the setup embed as a future feature, but the selectable option is not currently exposed by `/setup`.
