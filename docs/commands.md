...
| `/rank info <user>` | View XP rank card | `user` (required): must resolve to a current guild member |
...
Join-to-Create is shown in the setup embed as a future feature, but it is not currently selectable from `/setup`.
