...
3. Configure:
   ...
   - **Channel** — the channel intended for the ticket panel
   - **Role** — the support role that gets access to tickets

The setup flow stores configuration only; it does not automatically post a ticket panel. A panel message still needs a select menu with custom ID `ticket` so `src/components/selects/ticket-menu.js` can open the ticket modal. Verify ticket creation after setup: `src/components/modals/ticket-modal.js` currently uses the stored `Channel` value as the parent for created ticket channels even though the setup UI labels `Category` separately.
...
Any member who can see the ticket channel and click the close button can trigger the close flow; the button handler does not perform an additional support-role check.
...
1. An operator must create a `jtcsetups` record for the guild and hub voice channel
...
Current constraints:
- `/setup` displays Join-to-Create as "added at a later date" and does not expose the JTC select option.
- No `src/components/selects/jtcSSM.js` handler exists, so setup data must be seeded or created by another tool.
- Runtime ownership tracking uses an in-memory `Collection`; active temporary channel ownership is not rebuilt from MongoDB after restart.
- `src/events/Guild/jointocreate.js` reads `data.UserLimit`, but `prisma/schema.prisma` does not define a `UserLimit` field on `JTCSetup`.
...
No translation command is currently shipped. The installed `@iamtraction/google-translate` package is not exposed through a slash command, prefix command, or context menu in this source tree.
