...
This installs all runtime dependencies and the Prisma CLI (dev dependency). The Prisma client is not generated by a `postinstall` hook in this repo, so generate it after installing dependencies and again after any `prisma/schema.prisma` change:
...
`src/example.config.js` currently checks `process.env.PRODUCTION` by truthiness for the MongoDB URI. Because `.env` values are strings, `PRODUCTION=false` still selects `MONGODB_URI`; set it for local development or edit the generated `src/config.js` to use `DEV_MONGODB_URI`.
...
- After a fresh clone, run `npx prisma generate`; dependency install does not generate the Prisma client automatically.
- With `PRODUCTION=false`, also set `MONGODB_URI` or adjust `src/config.js`, because the template's MongoDB URI selector treats any non-empty `PRODUCTION` string as truthy.
