...
- **Node.js** v18 or higher. Node 22 matches the release workflow; `pretty-ms` 9.x and its `parse-ms` dependency require Node 18+.
...
This installs the committed lockfile exactly. Use `npm install` only when you intentionally need to refresh dependency versions.

The Prisma client is not generated by a `postinstall` hook in this repo. Generate it explicitly after installing dependencies or changing `prisma/schema.prisma`:
...
- `Running on http://127.0.0.1:8080` unless `HEALTH_HOST` overrides the bind host
