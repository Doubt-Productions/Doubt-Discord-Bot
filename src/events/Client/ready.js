...
module.exports = {
    ...
    run: (client) => {
        ...
        const clientData = [{
            ...
            ID: client.user.id,
            Members: client.guilds.cache.reduce((a, b) => a + b.memberCount, 0).toLocaleString('en'),
        ...
        const commandData = [
            {
                ...
                TotalCommands: client.collection.interactioncommands.size + client.collection.prefixcommands.size + client.collection.developercommands.size,
            }
        ]
        ...
        console.log(`Logged in on discord as: ${client.user.tag}`.replace(/(Logged in on discord as: )/, `${white + bold}$1${green}`).concat(reset));
    }
};